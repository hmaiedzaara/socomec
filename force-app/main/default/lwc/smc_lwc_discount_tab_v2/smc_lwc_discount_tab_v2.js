...
import getDiscountInfo from "@salesforce/apex/SMC_AC_Scoring.getTheScoring_v2"; //Check the controller + JS
import saveTheScoring from "@salesforce/apex/SMC_AC_Scoring.saveTheScoring"; //Check the controller + JS
import MESSAGE_NO_DATA from "@salesforce/label/c.SMC_No_Data_for_BA";


export default class Smc_lwc_discount_tab_v2 extends LightningElement {
  @api recordid;
  @api account;
  @api tokentab;
  @api baIds;
  ...
  save = { SAVE };
  messageNoData = { MESSAGE_NO_DATA };

  connectedCallback() {
    this.recordId = this.recordid;
    this.accountSerialized = JSON.stringify(this.account);
    console.log('this.baIds Tab ' + this.baIds);
    getDiscountInfo({
      ...
      if (result) {
        ...
        console.log('baIds Tab--> ' + this.baIds);
        console.log('scoringLines--> ' + result.scoringLines);
        /*There is 2 list :
            - scoring = memory list
        ...
        */
        ...
        if(this.manipulatedList.scoringLines.length > 0){
          this.isThereDiscount = true;
        } else {
      ...
      .catch(error => {
        ...
        console.log('ERROR CODE ++++ ' + error.body.errorCode);
        /*this.dispatchEvent(
          new ShowToastEvent({
            title: "Error",
    ...
    if(scoringShow.length > 0)
    {
      ...
      if ((search !== "") && (searchDescription === "") && (!checkOverMaxDiscount)) {
        for (var j = 0; j < scoringShow.length; ++j) {
          var strPriceGroup = scoringShow[j].priceGroup.code.toLowerCase(); //Price group - pricegroup.id
      ...
      if ((search === "") && (searchDescription !== "") && (!checkOverMaxDiscount)) {
        for (var j = 0; j < scoringShow.length; ++j) {
          var strDescription = scoringShow[j].priceGroup.descr.toLowerCase(); //Description - pricegroup.label
